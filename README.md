...
`run.cmd` は、`%HOME%\smt-mcp-server-poc.env` が存在する場合に `--env-file` として優先的に読み込みます。存在しない場合は、リポジトリルートの `.env` を読み込みます。

`%HOME%\smt-mcp-server-poc.env` または `.env` の例は次のとおりです。
...
ワークスペースルートには仮想の default ignore policy と default allow policy が存在します。default ignore policy では `.git`、`node_modules`、仮想環境、ビルド出力などが非公開になります。default allow policy では、ソースコード、設定ファイル、ドキュメント系の一般的な拡張子と `Dockerfile`、`Makefile`、`CMakeLists.txt`、`LICENSE` が許可されます。default allow policy の拡張子判定は pathspec として評価されるため、`.MD` や `.CMD` などの大文字拡張子は既定では許可されません。これらの仮想 policy はルートの `.mcpignore` より先に評価されるため、ルートの `.mcpignore` で `ignore **/*.md` のように指定すると、既定で許可される `README.md` も非公開にできます。

`ignore` 側の pattern には pathspec の negation を使用できます。たとえば、ルートの `.mcpignore` に `ignore !node_modules/` と `allow node_modules/**/*.ts` を指定すると、default ignore の `node_modules/` を打ち消したうえで、該当する TypeScript ファイルを公開できます。

ファイル単位の判定では、下位ディレクトリの `.mcpignore` から上位へ fallback します。下位の `.mcpignore` で `allow` または `ignore` が確定した場合は、その結果が使用されます。最後まで `allow` されないファイルは非公開です。

ディレクトリが `ignore` に一致した場合、その配下全体は非公開となり、子孫ディレクトリの `.mcpignore` による `allow` では復活できません。また、`.env`、秘密鍵、証明書、データベースファイル、`.mcpignore` などの hard deny 対象は、`.mcpignore` の `allow` では公開できません。
...
# サブプロジェクトで必要なファイルを公開します。
allow packages/editor/**/*.el
```
...
- シンボリックリンクは追跡されません。
- `.env`、秘密鍵、証明書、データベースファイル、`.mcpignore` などの hard deny 対象は公開されません。
- `.mcpignore` の公開ポリシーで allow されないファイルは公開されません。
